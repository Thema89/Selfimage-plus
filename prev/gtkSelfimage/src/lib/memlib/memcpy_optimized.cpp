...
// memcpy.cpp - Optimized memory copy
// This file was originally memcpy_amd.cpp
//---------------------------------------------------------------------------
// Copyright Note:
//
// This original copyright notice for this file is reproduced below.  The
// file has been extensively modified from the original AMD source.  AMD
// seems to have placed no restrictions on modification or distribution other
// than to indemnify themselves from any responsibility.
//
//---------------------------------------------------------------------------
// Changes from the original:
//
// 8 Oct 2003 - katsyonak <katsyonak@users.sourceforge.net>
//   Added MMX & SSE optimized memcpy - October 8, 2003
// 12 Oct 2003 - katsyonak <katsyonak@users.sourceforge.net>
//   Added AMD, MMX & SSE optimized memset - October 12, 2003
// 11 Feb 2004 - katsyonak <katsyonak@users.sourceforge.net>, and
//               Aw3 <aw3@users.sourceforge.net>
//   Added AMD, MMX & SSE optimized memzero - February 11, 2004
// 4 Dec 2005 - Kurt Fitzner <kfitzner@excelcia.org>
//   Modifications to compile under Borland C++ Builder/TASM
// 21 Feb 2007 - Kurt Fitzner <kfitzner@excelcia.org>
//   Further cleanup of earlier Borland C++ Builder/TASM port.
//   See memcpy_optimized.h for details
//   General file prettying up
//---------------------------------------------------------------------------
...
// FITNESS FOR ANY PARTICULAR PURPOSE.  IN NO EVENT SHALL AMD OR ITS
...
// EVEN IF AMD HAS BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.  BECAUSE
...
$History: * memcpy_optimized.cpp - 2005-12-04 10:37:36 AM - 35885 Bytes
$History: * memcpy_optimized.h - 2005-12-04 10:32:18 AM - 1638 Bytes
$History: * Initial check-in
$History: **** V 1.1 by kfitzner ****
$History: * memcpy_optimized.cpp - 2005-12-04 10:49:39 AM - 36202 Bytes
$History: * memcpy_optimized.h - 2005-12-04 10:49:04 AM - 712 Bytes
$History: * Make memlib.h the library header - memcpy_optimized.h 
$History: * is the unit header, though it doesn't really need one
$History: **** V 1.2 by kfitzner ****
$History: * memcpy_optimized.cpp - 2005-12-09 4:39:44 PM - 36979 Bytes
$History: * memcpy_optimized.h - 2005-12-04 10:49:04 AM - 712 Bytes
$History: * Clarify original filename in comments and add some 
$History: * sort of email address for the contributors.  Their 
$History: * SourceForge addresses were used because their normal 
$History: * contact information isn't known.
$History: **** V 1.3 by kfitzner ****
$History: * memcpy_optimized.cpp - 2007-03-30 7:09:22 PM - 41455 Bytes
$History: * memcpy_optimized.h - 2007-02-22 5:52:16 PM - 1679 Bytes
$History: * Correct ALIGN issues - manually align for 1.2
$History: **** Latest ** V 1.4 by kfitzner ** 2007-04-01 2:29:30 PM ****
$History: * Align for 1.2.1 release
*/
//----------------------------  $NoKeywords ---------------------------------
#pragma inline
//---------------------------------------------------------------------------
...
#pragma warn-8070  // Don't warn on functions not appearing to return values
unsigned long CPU_Type = 0;
unsigned long memcpyProc = 0;
unsigned long memsetProc = 0;
unsigned long memzeroProc = 0;
...
unsigned long get_cpu_type(void)
{
  __asm
  {
        mov         eax, [CPU_Type]
...
void* __stdcall memcpy_optimized(void *dest, const void *src, size_t n)
{
  __asm
  {
        // We need to make sure we're aligned on a 16-byte boundary right at the beginning so
        // we can set our start marker against which our CALIGN macro checks.
        nop
        nop
//      nop
//      nop
...
void* __stdcall memset_optimized(void *dest, int c, size_t n)
{
  __asm
  {
    ...
    $memset_detect:
        ...
        push        edi
        call        get_cpu_type
        mov         ecx, offset fill_sse1
        cmp         al, 3
        ja          addr_done2
        mov         ecx, offset fill_amd1
        je          addr_done2
        mov         ecx, offset fill_mmx1
        cmp         al, 1
        ja          addr_done2
        mov         ecx, offset fill_rep1
    addr_done2:
        mov         [memsetProc], ecx
        pop         edi
        pop         ebx
    ...
    fill_sse1:
        cmp         ebx, 2048
        jb          fill_mmx1        // tiny? skip optimized fill
    ...
    fill_amd1:
        cmp         ebx, 128
        jb          fill_rep1        // tiny? skip optimized fill
...
void __stdcall memzero_optimized(void *dest, size_t n)
{
  __asm
  {
    ...
    $memzero_detect:
        ...
        push        edi
        call        get_cpu_type
        mov         ecx, offset fill_sse2
        cmp         al, 3
        ja          addr_done3
        mov         ecx, offset fill_amd2
        je          addr_done3
        mov         ecx, offset fill_mmx2
        cmp         al, 1
        ja          addr_done3
        mov         ecx, offset fill_rep2
    addr_done3:
        mov         [memzeroProc], ecx
        pop         edi
        pop         ebx
    ...
    fill_sse2:
        cmp         ebx, 2048
        jb          fill_mmx2        // tiny? skip optimized fill
    ...
    fill_amd2:
        cmp         ebx, 128
        jb          fill_rep2        // tiny? skip optimized fill
